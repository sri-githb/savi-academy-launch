...
const Footer = () => {
  ...
  return <footer className={`relative pt-20 pb-8 overflow-hidden ${isDark ? '' : 'bg-navy'}`}>
      {/* Background */}
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              {socialLinks.map(social => <motion.a key={social.label} href={social.href} aria-label={social.label} className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-300 ${isDark ? 'bg-secondary/50 text-muted-foreground hover:text-primary hover:bg-primary/10' : 'bg-white/10 text-white/70 hover:text-white hover:bg-white/20'}`} whileHover={{
              scale: 1.1,
              y: -2
            }} whileTap={{
              scale: 0.9
